...
def get_vline_trains_7pm():
    """Get first 5 V/Line trains around 7 PM from Tarneit to Southern Cross"""
    ...
    try:
        ...
        for dep in departures_data['departures']:
            ...
            if scheduled:
                try:
                    ...
                    # Get trains within 3 hours of 7 PM
        ...
        first_5 = target_trains[:5]
        
        if not first_5:
            print("\n❌ No V/Line trains found around 7 PM")
            return
        
        print(f"\n📋 First 5 V/Line Trains Around 7:00 PM:\n")
        ...
        for i, train in enumerate(first_5, 1):
            ...
            mins_diff = (train['datetime'].hour * 60 + train['datetime'].minute) - (target_hour * 60 + target_minute)
            if mins_diff == 0:
                print(f"⏱️  Timing:    PERFECT! Exactly at 7:00 PM")
            elif mins_diff < 0:
                print(f"⏱️  Timing:    {abs(mins_diff)} minutes BEFORE 7:00 PM")
            else:
                print(f"⏱️  Timing:    {mins_diff} minutes AFTER 7:00 PM")
